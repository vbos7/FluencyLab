...
export function LevelCard({ level, xp, xpNeeded, streak }: Props) {
    ...
    return (
        <div className="relative overflow-hidden rounded-2xl p-7 mb-1 bg-gradient-to-br from-blue-600 to-blue-300 shadow-xl shadow-blue-400 text-white">
            ...
            <div className="relative z-10">
                ...
                <div className="flex justify-between items-start mb-6">
                    ...
                    <div className="flex items-center gap-2 px-3.5 py-2 rounded-xl text-sm font-bold bg-white/15 border border-white/20 backdrop-blur-sm">
                        <FlameIcon />
                        {streak} dias streak
                    </div>
                </div>
                ...
                <div className="h-2.5 bg-white/20 rounded-full overflow-hidden">
                    <div
                        className="h-full rounded-full bg-gradient-to-r from-white/90 to-white/60 relative transition-all duration-1000 ease-out"
                        style={{ width: `${pct}%` }}
                    >
                        <span className="absolute right-0 top-1/2 -translate-y-1/2 w-3.5 h-3.5 bg-white rounded-full shadow-[0_0_10px_rgba(255,255,255,0.8)]" />
                    </div>
                </div>

                <p className="mt-2.5 text-right text-[0.7rem] opacity-60 font-medium">
                    Faltam <strong className="opacity-100">{xpNeeded - xp} XP</strong> para o Nível {level + 1} 🚀
                </p>
            </div>
        </div>
    )
...
function FlameIcon() {
    return (
        <svg
            ...
            style={{ animation: "flameSway 1.8s ease-in-out infinite alternate", transformOrigin: "50% 95%" }}
        >
            <defs>
            ...
            <path
                ...
                style={{ animation: "flameBreath 0.9s ease-in-out infinite alternate", transformOrigin: "16px 36px" }}
            />
            {/* Chama intermediária */}
            <path
                ...
                fill="url(#flameInner)"
                style={{ animation: "flameBreath 0.7s 0.1s ease-in-out infinite alternate", transformOrigin: "16px 34px" }}
            />
            {/* Núcleo */}
            <path
                ...
                fill="url(#flameCore)"
                style={{ animation: "coreFlicker 0.5s 0.05s ease-in-out infinite alternate", transformOrigin: "16px 29px" }}
            />
            {/* Faíscas */}
            <circle cx="9"  cy="10" r="1"   fill="#fb923c" style={{ animation: "sparkFloat 1.4s 0.0s ease-out infinite", opacity: 0 }} />
            <circle cx="23" cy="8"  r="0.8" fill="#fbbf24" style={{ animation: "sparkFloat 1.2s 0.3s ease-out infinite", opacity: 0 }} />
            <circle cx="7"  cy="15" r="0.7" fill="#f97316" style={{ animation: "sparkFloat 1.6s 0.6s ease-out infinite", opacity: 0 }} />
            <circle cx="25" cy="13" r="1"   fill="#fde68a" style={{ animation: "sparkFloat 1.1s 0.9s ease-out infinite", opacity: 0 }} />
            <circle cx="13" cy="6"  r="0.6" fill="#fb923c" style={{ animation: "sparkFloat 1.3s 0.2s ease-out infinite", opacity: 0 }} />
        </svg>
    )
}
