...
export default function CursoClient({ course }: { course: CourseDetail }) {
    ...
    fetch(`${API_URL}/auth/me.php`, { credentials: "include" })
        ...
        .then((data) => setUsuarioLogado(data ?? null)) // 👈 usa o objeto direto, sem .user
    ...
    async function enviarComentario() {
        console.log("usuarioLogado:", usuarioLogado, "aula:", aula?.id, "texto:", novoComentario);
                        ...
                        <div className="relative ml-auto w-72 h-full bg-white shadow-xl flex flex-col">
                            ...
                            <div className="flex-1 overflow-y-auto py-2">
                                {course.lessons.map((l, i) => (
                                    ...
                                    >
                                        <span className={`shrink-0 w-6 h-6 rounded-full flex items-center justify-center text-[11px] font-semibold mt-0.5 ${aulaAtiva === i ? "bg-blue-600 text-white" : "bg-gray-100 text-gray-500"}`}>
                                            {i + 1}
                                        </span>
                                        <div>
                                            <p className={`text-sm font-medium leading-snug ${aulaAtiva === i ? "text-blue-700" : "text-gray-800"}`}>{l.title}</p>
                        ...
                        <div className="flex-1 min-w-0 flex flex-col gap-5">
                            <div className="bg-white rounded-2xl border border-[#dce8ff] shadow-[0_2px_16px_rgba(37,99,235,0.08)] overflow-hidden">
                                ...
                                <div className="px-5 py-4 flex items-center justify-between">
                                    <div>
                                        <p className="text-sm font-semibold text-gray-900">{aula.title}</p>
                            ...
                            <div className="bg-white rounded-2xl border border-[#dce8ff] shadow-[0_2px_16px_rgba(37,99,235,0.08)] px-5 py-5">
                                <div className="flex items-center gap-2 mb-5">
                                    ...
                                    <p className="text-sm font-semibold text-gray-900">Comentários <span className="text-gray-400 font-normal">({comentarios.length})</span></p>
                                </div>
                                <div className="flex gap-3 mb-6">
                                    ...
                                    <div className="flex-1 flex items-center gap-2 border border-gray-200 rounded-xl px-3 py-2 focus-within:border-blue-400 focus-within:ring-2 focus-within:ring-blue-50 transition">
                                        ...
                                        />
                                        <button onClick={enviarComentario} disabled={!novoComentario.trim()} className="text-blue-500 hover:text-blue-700 disabled:opacity-30 transition"><Send size={15} /></button>
                                    </div>
                                </div>
                                <div className="flex flex-col gap-4">
                                    ...
                                    {comentarios.map((c) => (
                                        <div key={c.id} className="flex gap-3">
                                            <div className="w-8 h-8 rounded-full bg-gray-100 text-gray-600 flex items-center justify-center text-xs font-semibold shrink-0">{c.user_name?.slice(0, 2).toUpperCase()}</div>
