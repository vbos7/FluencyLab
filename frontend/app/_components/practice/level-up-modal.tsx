...
export function LevelUpModal({ level, onClose }: Props) {
    ...
    return createPortal(
        ...
        >
            ...
            <div
                className={`relative w-full max-w-sm ${reduce ? "" : "animate-in fade-in zoom-in-95 duration-300"}`}
            >
                <div
                    className={`overflow-hidden rounded-3xl bg-white text-center shadow-[0_24px_64px_rgba(37,99,235,0.35)] ${reduce ? "" : "animate-[levelUpShake_2s_ease-in-out_infinite]"}`}
                >
